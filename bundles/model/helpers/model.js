...
class ModelHelper extends helper {
  ...
  deafen(sessionID, listenModel, listenID) {
    ...
    // Call local
    return this.eden.thread('compute', 0).call('model.deafen', sessionID, listenModel.constructor.name.toLowerCase(), listenModel.get('_id').toString(), listenID);
  }
  ...
  listen(sessionID, listenModel, listenID, atomic = false) {
    ...
    // Call local
    return this.eden.thread('compute', 0).call('model.listen', sessionID, listenModel.constructor.name.toLowerCase(), listenModel.get('_id').toString(), listenID, atomic);
  }
}
